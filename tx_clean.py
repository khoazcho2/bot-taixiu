...
async def _tx_countdown(ctx, cid):
    ...
    async def countdown_job(context):
        ...
        if seconds <= 0:
            context.job.schedule_removal()
            await _tx_ket_thuc(context, cid)
            return

        try:
            await msg.edit_text(header + f"⏳ {seconds}...")
        except:
            pass
